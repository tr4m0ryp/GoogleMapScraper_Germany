...
# Load Germany's borders from a GeoJSON file
GEOJSON_FILE_PATH = 'path_to_germany_geojson.geojson'  # Update with the actual path

console.log("[bold blue]Starting the process for Germany...[/bold blue]")
...
if not os.path.exists(GEOJSON_FILE_PATH):
    ...
    try:
        ...
        GERMANY_BORDERS = shape(geojson_data['features'][0]['geometry'])
    ...
    while True:
        try:
            ...
            if 'OVER_QUERY_LIMIT' in str(e) or 'REQUEST_DENIED' in str(e) or 'BILLING' in str(e):
                ...
                save_and_prompt(places, "Partial_Data_Germany.xlsx")
                break
...
def main():
    console.log("[bold blue]Starting main process for Germany...[/bold blue]")
    api_key = API_KEY
    if not api_key:
    ...
    coordinates = generate_grid_coordinates(GERMANY_BORDERS, NUM_GRIDS)
    if not coordinates:
    ...
    if all_data:
        filename = f"{query}_in_Germany.xlsx"
